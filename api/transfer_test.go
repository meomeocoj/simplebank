	...
	testCases := []struct {
	...
	}{
		{
			...
			setupAuth: func(t *testing.T, req *http.Request, tokenMaker token.Maker) {
				addHeader(t, req, tokenMaker, time.Minute, authorizationTypeBearer, user1.Username)
			},
			buildStubs: func(store *mockdb.MockStore) {
		...
		},
		{
			...
			setupAuth: func(t *testing.T, req *http.Request, tokenMaker token.Maker) {
				addHeader(t, req, tokenMaker, time.Minute, authorizationTypeBearer, user2.Username)
			},
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account1.ID)).Times(1).Return(account1, nil)
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account1.ID)).Times(1).Return(account1, nil)
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account2.ID)).Times(1).Return(account2, nil)
				store.EXPECT().TransferTransaction(gomock.Any(), gomock.Any()).Times(1).Return(db.TransferTxResult{}, sql.ErrTxDone)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusInternalServerError, recorder.Code)
			},
